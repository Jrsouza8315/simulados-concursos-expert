...
import { BookOpen, Clock, Trophy, BarChart3, Download, Star } from 'lucide-react';
...
const AssinanteDashboard = () => {
  ...
  const recentSimulados = [
    { id: 1, title: "TRT - Analista Judiciário", score: 85, date: "2024-01-15" },
    { id: 2, title: "Receita Federal - Auditor", score: 92, date: "2024-01-10" },
      ...
      <div className="pt-20 px-4 sm:px-6 lg:px-8">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            ...
            <Card className="border-l-4 border-l-secondary-500">
              ...
              <CardContent>
                ...
                <p className="text-xs text-muted-foreground">+5% desde o início</p>
              </CardContent>
            </Card>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
            ...
            <Link to="/apostilas">
              <Button variant="outline" className="w-full h-20 flex-col border-secondary-300 text-secondary-600 hover:bg-secondary-50">
                <Download className="h-6 w-6 mb-2" />
                Apostilas Exclusivas
              </Button>
            </Link>
            <Button variant="outline" className="w-full h-20 flex-col border-accent-300 text-accent-600 hover:bg-accent-50">
              ...
              <CardContent>
                <div className="space-y-4">
                  {recentSimulados.map((simulado) => (
                    <div key={simulado.id} className="flex items-center justify-between p-3 border rounded-lg hover:bg-gray-50">
                      <div>
                        <p className="font-medium text-primary-900">{simulado.title}</p>
                        <p className="text-sm text-gray-500">{simulado.date}</p>
                      </div>
                      <div className="text-right">
                        <p className={`text-lg font-bold ${simulado.score >= 80 ? 'text-green-600' : simulado.score >= 60 ? 'text-yellow-600' : 'text-red-600'}`}>
                          {simulado.score}%
                        </p>
                      </div>
                    </div>
